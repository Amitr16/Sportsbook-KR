...
def get_theme_css(subdomain):
    ...
    try:
        ...
        # Get operator ID from subdomain
        print(f"🔍 Looking up operator for subdomain: {subdomain}")
        cursor.execute('SELECT id FROM sportsbook_operators WHERE subdomain = ?', (subdomain,))
        operator = cursor.fetchone()
        ...
        print(f"✅ Found operator: {operator[0]}")
        
        # Get theme customization
        print(f"🔍 Looking up theme for operator_id: {operator[0]}")
        cursor.execute('''
            SELECT * FROM sportsbook_themes 
            WHERE sportsbook_operator_id = ?
        ''', (operator[0],))
        ...
        theme = cursor.fetchone()
        print(f"🔍 Theme found: {theme is not None}")
        if theme:
            print(f"🔍 Theme colors: primary={theme[3]}, secondary={theme[4]}")
        ...
        if not theme:
            ...
            css = f"""
            :root {{
                --primary-color: {theme[3]};
                --secondary-color: {theme[4]};
                --accent-color: {theme[5]};
