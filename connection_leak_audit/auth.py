        ...
        try:
            ...
            with connection_ctx(timeout=3) as conn:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute("SELECT * FROM users WHERE id = %s LIMIT 1", (data['user_id'],))
                    user_row = cursor.fetchone()
                    current_user = user_row if user_row else None
    ...
    try:
        ...
        with connection_ctx(timeout=5) as conn:
            with conn.transaction():
                cursor.execute("""
                    INSERT INTO users (username, email, password_hash, balance, is_active, created_at)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    RETURNING id
                """, (username, email, password_hash, default_balance, True, datetime.now()))
                user_id = cursor.fetchone()['id']
        
        user = {'id': user_id, 'username': username, 'email': email, 'balance': default_balance}
...
def get_current_user_jwt():
    ...
    try:
        ...
        with connection_ctx(timeout=3) as conn:
            with conn.cursor() as cursor:
                cursor.execute("SET LOCAL statement_timeout = '2000ms'")
                cursor.execute("SELECT * FROM users WHERE id = %s LIMIT 1", (g.current_user.id,))
                user_row = cursor.fetchone()
