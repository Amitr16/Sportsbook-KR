@echo off
REM Set database pool environment variables for Fly.io
REM This allows the pool to shrink during quiet periods instead of growing forever

echo Setting database pool environment variables...
echo.

REM Let pool go down to zero when idle (instead of keeping minimum 2)
fly secrets set DB_POOL_MIN=0

REM Reap idle connections after 60 seconds (instead of 300)
fly secrets set DB_POOL_MAX_IDLE=60

REM Optional: Churn older sockets every 10 minutes for connection health
